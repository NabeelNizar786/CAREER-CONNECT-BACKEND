...
const empRegister = async (req,res) => {
  try {
    ...
    await newEmp.save()
    res.status(200).json({message:"REGISTRATION SUCCESSFULL",created:true})
...
const empLogin = async (req, res) => {
  try {
    ...
    const isMatch = await empModel.findOne({email: email, password:sha256(password+process.env.PASSWORD_SALT) });
    // console.log(isMatch);
    if (!isMatch) {
      return res
        .status(401)
...
const isEmpAuth = async(req,res) => {
  try {
    ...
    if(!empData) {
      ...
      return res
      .status(200).json({success: true, empData:empData});
    }
  } catch (error) {
    console.log(error);
    return res
...
const forgotPassword = async (req,res) => {
  try {
    ...
    if(check === 'yes') {
      return res
      .status(404)
      .json({message: "EMPLOYER DOES NOT EXIST", check:false})
    } 
